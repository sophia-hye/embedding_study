...
class CNN(object):
  ...
  def __init__(self, train_size, embedding_size, channel_size,
               dropout_prob, filter_sizes, num_filters, threshold, max_token_length):
    ...
    #self.input_x = tf.expand_dims(self.input_x, -1) # channel  차원 추가
    ...
    with tf.name_scope('dropout'):
      self.h_drop = tf.nn.dropout(self.h_pool_flat, dropout_prob)
    ...
    with tf.name_scope('loss'):
      losses = []
      """
      batch_size, n_elems = self.vectors.get_shape()
      idxs = list(itertools.permutations(range(n_elems)))
      permutations = tf.gather(self.vectors, idxs, axis=-1) # Shape=(batch_size, n_permutations, n_elems)
      mse = tf.square(permutations - self.input_y[:, None, :]) # Shape=(batch_size, n_permutations, n_elems)
      self.loss = tf.reduce_mean(mse)
      """

      """
      for y_pred, y_true in zip(self.vectors, self.input_y):
        loss = 0
        for i in range(0, 300):
          loss += tf.square(y_pred[i]-y_true[i])
        loss = loss / 300
        losses.append(loss)
      self.loss = tf.reduce_mean(losses)
      """
      self.loss = tf.reduce_sum(tf.square(self.vectors - self.input_y), axis=1) / embedding_size
    ...
    with tf.name_scope('accuracy'):
      predictions = []
      #cos_sim = tf.tensordot(self.vectors, self.input_y, 1)/(tf.norm(self.vectors)*tf.norm(self.input_y))
      
      #cosine_loss = tf.keras.losses.CosineSimilarity(axis=1)
      #cos_sim = cosine_loss(self.input_y, self.vectors)
      
      cos_similarity = tf.reduce_sum(tf.multiply(tf.norm(self.vectors), tf.norm(self.input_y)))
      correct_predictions = tf.math.greater_equal(cos_similarity, self.threshold)
      self.accuracy = tf.reduce_mean(tf.cast(correct_predictions, 'float'), name='accuracy')
      """
      isYes = []
      for y_pred, y_true in zip(self.vectors, self.input_y):
        a = np.array(y_pred, dtype=float)
        b = np.array(y_true, dtype=float)
        cos_sim = tf.inner(a, b)/(norm(a)*norm(b))
        if cos_sim >= self.threshold: isYes.append(1)
        else: isYes.append(0)
      self.accuracy = tf.reduce_mean(tf.cast(isYes, 'float'), name='accuracy')
      """
...
x_train_input, x_test_input = get_x_input(x_train), get_x_input(x_test)
...
with tf.Graph().as_default():
  ...
  with sess.as_default():
    model = CNN(train_size=train_size,
                ...
                num_filters=128,
                threshold=0.80, # cosine similarity threshold (accuracy)
    ...
    for (x_batch, y_batch) in batch_iter(x_train_input, y_train_input, batch_size, 10):
      ...
      if current_step % evaluate_every == 0:
        if testpoint + aPoint < len(x_test_input):
          ...
          testpoint = 0
        print("\nEvaluation:")
        dev_step(x_test_input[testpoint:testpoint+aPoint], y_test_input[testpoint:testpoint+aPoint], writer=dev_summary_writer)
        print("")
