...
class CNN(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, nf, kernel, stride, padding):
        ...
        self.net = nn.Sequential(
            ...
            nn.AvgPool1d(2),
            nn.ReLU(True),
            nn.Conv1d(self.nf * 2, self.nf, self.kernel, self.stride, self.padding, bias=False),
            nn.AvgPool1d(2),
            nn.ReLU(True),
            nn.Conv1d(self.nf, self.out_dim, self.kernel, self.stride, self.padding, bias=False),
            nn.Tanh()  # range -1 ~ 1
        )
