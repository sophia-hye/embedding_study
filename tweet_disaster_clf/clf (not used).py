...
def run_model(model, lr, loss_fn, optimizer, train_loader, test_loader):
  ...
  for epoch in range(500):
    ...
    if (epoch+1) % 50 == 0:
      print("{:3d}-epoch | trn loss: {:2.2f} | trn best accuracy: {:.4f} | val loss: {:2.2f} | val best accuracy: {:.4f}".format(epoch+1, trn_avg_loss, best_trn_accuracy, avg_loss, best_accuracy))
...
if __name__ == '__main__':
    ...
    new_embedding_list = pd.read_csv('prediction_embedding_keyword_model.csv')
    ...
    #------------------------------------------------------------------------------------------------------------------
    # disaster tweets data: conceptnet
    for i in range(1):
        KEY = 'conceptnet'
        ...
        best_accuracy = run_model(model, 3e-2, loss_fn, optimizer, train_loader, test_loader)
